...
if [ ! $# -eq 4 ];
then
    echo "run_gem5_spec17_benchmarks.sh <BENCHMARK> <OUTDIR> <MAXINST> <l3size>"
...
# Run gem5
${GEM5OPT} --outdir=${OUTDIR} --debug-flags=DynamicCacheCtrl ${SPEC17CFG} -b ${ACTUALBENCHNAME} -i ${MAXINST} -l ${SIZE}
